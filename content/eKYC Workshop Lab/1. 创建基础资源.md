...
可以在 Cloud9 中，运行以下命令创建 IAM Role 并添加 Policy，也可以参考后面的截图在 AWS 控制台中创建 Role。
```
...
aws iam attach-role-policy --role-name ekyc-Lambda-role --policy-arn arn:aws:iam::aws:policy/AWSLambdaSQSQueueExecutionRole
...
**如果运行以下 CLI 出现报错 Invalid Token，请在 Cloud9 设置 -- AWS Settings 中，关闭 AWS managed temporary credentials**
```
...
aws iam attach-role-policy --role-name ekyc-StepFunctions-role --policy-arn arn:aws:iam::aws:policy/AmazonTextractFullAccess
